...
(define math-exp->firstNum
  (lambda (math-exp)
    (cond
      ((lit-exp? (car(cdr math-exp))) (literal-exp->value (car(cdr math-exp))))
      ((var-exp? (car(cdr math-exp))) (apply-env (var-exp->var-name (car(cdr math-exp))) env)))))
(define math-exp->secondNum
  (lambda (math-exp)
    (cond
      ((lit-exp? (car(cdr(cdr(cdr math-exp))))) (literal-exp->value (car(cdr(cdr(cdr math-exp))))))
      ((var-exp? (car(cdr(cdr(cdr math-exp))))) (apply-env (var-exp->var-name (car(cdr(cdr(cdr math-exp))))) env)))))
; math-ex e - 2
;do math helper function
(define do-math 
  (lambda(math-exp)
    (cond
      ((eq? (math-exp->operator math-exp) +)
       (+ (math-exp->firstNum math-exp)(math-exp->secondNum math-exp)))
      ((eq? (math-exp->operator math-exp) -)
       (- (math-exp->firstNum math-exp)(math-exp->secondNum math-exp)))
      ((eq? (math-exp->operator math-exp) *)
       (* (math-exp->firstNum math-exp)(math-exp->secondNum math-exp)))
  ...
  (lambda (c0d3)
    (cond
      ((eq? (car c0d3) 'math) (math-exp
                               (parse-expression (car(cdr c0d3)))
                               (car(cdr(cdr c0d3)))
                               (parse-expression (car(cdr(cdr(cdr c0d3)))))))
